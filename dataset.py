...
def get_dataloader(
    ...
    if dali:
        return dali_data_iter(
            batch_size=batch_size, num_threads=num_workers, data_dir=root_dir, 
            local_rank=local_rank, name="reader", dali_aug=dali_aug)
    transform = transforms.Compose([
            transforms.RandomHorizontalFlip(),
            transforms.RandomRotation(180),
            transforms.RandomResizedCrop(size=448, scale=(0.6, 1.)),
            transforms.ToTensor(),
...
def dali_data_iter(
    batch_size: int, num_threads: int, data_dir=None,
    initial_fill=32768, random_shuffle=True,
    prefetch_queue_depth=4, local_rank=0, name="reader",
    mean=(127.5, 127.5, 127.5), 
    std=(127.5, 127.5, 127.5),
    dali_aug=True, dist=True, image_size=448
    ):
    """
    Parameters:
    ----------
    ...
    """
    rank: int = distributed.get_rank() if dist else 0
    world_size: int = distributed.get_world_size() if dist else 1
    ...
    def dali_random_rotation(img, rotation_angle):
        img = fn.rotate(img, angle=rotation_angle, fill_value=0, keep_size=True)
        return img
    ...
    hsv_hue = fn.random.uniform(range=(-0.8, 0.8), dtype=types.FLOAT)
    window_size_blur = fn.random.uniform(range=(1, 2), dtype=types.INT32)
    hsv_saturation = fn.random.uniform(range=(0.8, 1.2), dtype=types.FLOAT)
    brightness = fn.random.uniform(range=(0.8, 1.2), dtype=types.FLOAT)
    contrast = fn.random.uniform(range=(0.8, 1.2), dtype=types.FLOAT)
    condition_rotation = fn.random.coin_flip(probability=0.5)
    rotation_angle = fn.random.uniform(range=(-180, 180), dtype=types.FLOAT)
    condition_flip = fn.random.coin_flip(probability=0.2)

    pipe = Pipeline(
        batch_size=batch_size, num_threads=num_threads,
        device_id=local_rank, prefetch_queue_depth=prefetch_queue_depth, )
    with pipe:
        # read jpegs and labels from folder
        jpegs, labels = fn.readers.file(
            file_root=data_dir, shard_id=rank, num_shards=world_size, 
            random_shuffle=random_shuffle, pad_last_batch=False, name=name)
        images = fn.decoders.image(jpegs, device="mixed", output_type=types.RGB)
        images = fn.cast(images, dtype=types.FLOAT)
        images = multiplexing(condition_rotation, dali_random_rotation(images, rotation_angle), images)
        ...
        images = brightness_contrast(images, brightness, contrast)
        images = fn.cast(images, dtype=types.UINT8)
        images = fn.random_resized_crop(images, size=(image_size, image_size), random_area=(0.6, 1.0), random_aspect_ratio=(1., 1.))
